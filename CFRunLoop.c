...
// 获取线程对应的runloop最终调用的核心函数  _CFRunLoopGet0
/*
这段代码可以看出：
...
// 在CFRunLoopGetCurrent函数内部调用了_CFRunLoopGet0()，传入的参数是当前线程。
// 这里可以看出，CFRunLoopGetCurrent函数必须要在线程内部调用，才能获取当前线程的RunLoop。也就是说子线程的RunLoop必须要在子线程内部获取
...
static void __CFRUNLOOP_IS_CALLING_OUT_TO_AN_OBSERVER_CALLBACK_FUNCTION__(CFRunLoopObserverCallBack func, CFRunLoopObserverRef observer, CFRunLoopActivity activity, void *info) {
    // 如果observer有回调函数，那么久调用这个回调函数，参数是观察者、runloop状态、runloop.observer.context上下文信息
    if (func) {
...
static Boolean __CFRunLoopDoBlocks(CFRunLoopRef rl, CFRunLoopModeRef rlm) { // Call with rl and rlm locked
    ...
    Boolean did = false;
    // 获取runloop的blocks_head头尾指针
    struct _block_item *head = rl->_blocks_head;
    struct _block_item *tail = rl->_blocks_tail;
    // 清空runloop的blocks_head头尾指针
    ...
    while (item) {
        ...
        item = item->_next;
	Boolean doit = false;
	// 如果block执行的的mode和当前mode一致，则执行block
	if (CFStringGetTypeID() == CFGetTypeID(curr->_mode)) {
...
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {	/* DOES CALLOUT */
    ...
    for (CFIndex idx = 0; idx < obs_cnt; idx++) {
        ...
        if (__CFIsValid(rlo)) {
            ...
            __CFRunLoopObserverUnlock(rlo);
            // 通知observer runloop当前所处的activity状态
...
static Boolean __CFRunLoopDoSources0(CFRunLoopRef rl, CFRunLoopModeRef rlm, Boolean stopAfterHandle) {	/* DOES CALLOUT */
	...
	if (CFGetTypeID(sources) == CFRunLoopSourceGetTypeID()) {
	    ...
	    __CFRunLoopSourceLock(rls);
	        ...
	        if (__CFIsValid(rls)) {
	            __CFRunLoopSourceUnlock(rls);
                // 触发非基于port的source0的回调
                // 其中rls->context.version0.perform是一个函数指针
    ...
    for (;;) {		/* In that sleep of death what nightmares may come ... */
        ...
        *voucherState = voucher_mach_msg_adopt(msg);
        ...
        if (MACH_MSG_SUCCESS == ret) {
            // 把msg中的msgh_local_port赋值给livePort
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    // runloop停止了则复位参数 然后返回stop终止runloop
	...
	__CFRunLoopDoBlocks(rl, rlm);
		// 4. RunLoop 触发 Source0 (非基于port的source) 回调
        ...
        if (MACH_PORT_NULL != dispatchPort && !didDispatchPortLastTime) {
            ...
            // __CFRunLoopServiceMachPort函数内部调用了mach_msg
	...
	__CFRunLoopUnlock(rl);
        ...
        do {
            ...
            msg = (mach_msg_header_t *)msg_buffer;
            /// 7. 调用 mach_msg 等待接受 mach_port 的消息。线程将进入休眠，等待被唤醒。 会被下面某一个事件唤醒:
            /// 一个基于 port 的Source 的事件（source1）。
            /// 一个 Timer 到时间了（处理timer）
            /// RunLoop 自身的超时时间到了（几乎不可能）
            /// 被其他什么调用者手动唤醒
